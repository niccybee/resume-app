...
<template><nav><ul><li><RouterLink role="button" to="/app/cvs/new">Create CV</RouterLink></li></ul></nav><p v-if="status==='loading'" aria-busy="true">Loading saved CVs…</p><section v-else-if="status==='empty'" class="empty-state"><h2>No saved CVs yet</h2><p>Create a role-focused CV from your CV Blocks.</p><RouterLink role="button" to="/app/cvs/new">Create the first CV</RouterLink></section><div v-else-if="status==='failed'" role="alert"><p>{{ error }}</p><button class="secondary" @click="load">Try again</button></div><section class="cv-list"><article v-for="cv in cvs" :key="cv.id"><p class="status">{{ cv.status }}</p><h2>{{ cv.name }}</h2><p>{{ resolveTheme(cv.themeId).name }} theme</p><footer><RouterLink :to="`/app/cvs/${cv.id}`">Edit CV</RouterLink><RouterLink :to="`/app/cvs/${cv.id}/preview`">Private preview</RouterLink><RouterLink v-if="cv.status==='published'" :to="`/cv/${cv.slug}`">Public link</RouterLink></footer></article></section></template>
